...
/**
  ...
  */
  ...
  override def externalTypeDeclaration(extType: ExternalType): Unit = extType match {
    case ExternalUserType(classSpec) if classSpec.meta.isOpaque =>
      opaqueClassDeclaration(classSpec)
    case _ =>
  ...
  private sealed trait NeedRaw {
    def level: Int
    def hasIO: Boolean
    def hasRaw: Boolean
  }
  private case object NotRaw extends NeedRaw {
    val level = 0
    val hasIO = false
    val hasRaw = false
  }
  private case object RawIo extends NeedRaw {
    val level = 1
    val hasIO = true
    val hasRaw = true
  }
  private case object RawProcess extends NeedRaw {
    val level = 1
    val hasIO = false
    val hasRaw = true
  }
  private case object RawIoProcess extends NeedRaw {
    val level = 2
    val hasIO = true
    val hasRaw = true
  }

  private def needRaw(dataType: DataType): NeedRaw = {
  ...
  override def condRepeatInitAttr(id: Identifier, dataType: DataType): Unit = {
    val needRaw = this.needRaw(dataType)
  ...
  /**
    ...
    */
  def createBuilderMap(curClass: ClassSpec, firstSpec: ClassSpec): Unit = {
...
object AwkwardCompiler extends LanguageCompilerStatic
  ...
  def outFileNameHeader(className: String): String = className + ".h"
